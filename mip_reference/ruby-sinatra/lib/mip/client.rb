...
module MIP
  ...
  class Client
    ...
    def request_connection(target_url, endorsements: [])
      path = extract_path(target_url) + '/mip_connections'
      payload = {
    ...
    def post_request(url, payload, include_public_key: false)
      ...
      signature = MIP::Signature.sign_request(@identity.private_key, timestamp, path, json_body)
    ...
    def get_request(url)
      timestamp = Time.now.iso8601
      path = extract_path(url)
      signature = MIP::Signature.sign_request(@identity.private_key, timestamp, path)
