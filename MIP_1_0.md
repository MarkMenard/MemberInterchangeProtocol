...
#### End Point: /mip/organizations/<organization_mip_id>/mip_connections
...
- mip_id: The MIP IDENTIFIER of the requesting node.
...
- MIP IDENTIFIER (of the node you are verifying a signature request for, not the MIP IDENTIFIER from the URL)
- Vouching Request
...
- group_id: The id of the group you are attempting to connect to on the foreign system. This would be provided by the person sending a request.
- mip_id: The MIP IDENTIFIER of the requesting organization.
...
A Signature Verification Request is a combination of a 128 bit random string (The Document) signed by the source parties private key, the signature, and their MIP IDENTIFIER. A Signature Verification Request can be sent to a third party as a means of verifying that that The Document was signed by the organization represented by the MIP IDENTIFIER. 

The payload for a request is:

- MIP IDENTIFIER
- Signature Message (a 128 bit random number represented in hexadecimal)
- RSA Signature (base 64 string encoded without new lines)

Example:

```json
{
  "mip_id": "512ef14957203c6323e79937f3935708",
  "document": "fce7240f-9e9d-4f12-9318-a6a0c1647e45",
